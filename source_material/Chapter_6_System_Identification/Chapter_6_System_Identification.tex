...
\subsubsection{Step Response Analysis
  (\texorpdfstring{$\zeta \ll 1$}{zeta much less than 1})}
			\begin{figure}[H]
				\centering
...
\begin{example}[Second-Order Performance Indicators in Simulink]
...
Logarithmic decrement ($\zeta \ll 1$) For a vibrating system, the mass ($m$) and stiffness ($k$) can be measured using scales and static deflection tests. However, the damping coefficient ($c$) is a more difficult quantity to determine. From $k$ and $m$ we can compute the natural frequency ($\omega_\text{n}$) and the critical damping coefficient ($c_\text{cr}$). Therefore, knowing that the critical damping ratio ($\zeta$) is defined as:
			\begin{equation}
...
\begin{example}[Damping from Logarithmic Decrement]
...
\begin{example}[Vehicle Suspension Damping]
