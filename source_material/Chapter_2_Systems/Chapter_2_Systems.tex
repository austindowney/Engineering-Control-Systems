...
\section{Systems}
	...
	\subsection{Single Degree-of-Freedom Systems}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_1dof_examples.pdf}
		...
		\subsubsection{Spring-Mass Model}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_spring_mass_point_mass.pdf}
		...
		\subsubsection{Linear Springs}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_linear_spring_deformation.pdf}
	...
	\subsubsection{Equation of Motion for an Oscillating System}			
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_horizontal_spring_mass_eom.pdf}
		...
		\begin{review}[Second-Order Homogeneous Differential Equations]
		...
		\begin{example}[Horizontal Spring--Mass Equation of Motion]
			
			Considering the system:
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_ex01_horizontal_system.pdf}
			\end{figure}		
			
			\examplestep{Define the displacement direction and draw the free-body diagrams.}
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_ex01_horizontal_fbd.pdf}
			\end{figure}		
			...
			\examplestep{Apply Newton's second law to the displaced state.}
			\begin{equation}
			...
			m\ddot{x} = -kx
			\end{equation}				
			\examplestep{Rearrange the result into standard equation-of-motion form.}
			\begin{equation}
			m\ddot{x} + kx = 0
			\end{equation}		
		\end{example}			
		...
		\begin{example}[Initial Displacement of a Vertical Spring--Mass System]
			Some systems have an initial displacement. Because the system oscillates about this position, the equation of motion must be defined relative to that position. Consider the following system:
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_ex02_vertical_system.pdf}
			\end{figure}		
			\examplestep{Define the displacement direction and draw the free-body diagrams.}
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_ex02_vertical_fbd.pdf}
			\end{figure}		
			\noindent The equation for the equilibrium state is:
			\begin{equation}
...
A step function is a common loading model for an input that changes abruptly from zero to a constant value. It can represent dropping a load into a truck, a car encountering a sudden change in the road, or a motor starting. The unit step, denoted by $\Phi(t)$ and also known as the Heaviside function, is defined as
\begin{equation}
	\Phi(t)=
	\begin{cases}
		0, & t<0,\\
		1, & t\geq 0.
	\end{cases}
	\label{eq:unit_step_definition}
\end{equation}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_unit_step.pdf}
...
A pulse function, denoted by $\rho(t;\tau)$, consists of a step up at $t=0$, followed by a step down at $t=\tau$. The pulse amplitude is $1/\tau$, so its width and height have a product of one. Consequently, changing the pulse duration changes its height while preserving unit area. A pulse can represent an actuator applying an approximately constant force for a fixed time before switching off:
\begin{equation}
	\rho(t;\tau)=
	\begin{cases}
		\dfrac{1}{\tau}, & 0\leq t<\tau,\\
		0, & \text{otherwise}.
	\end{cases}
	\label{eq:unit_pulse_definition}
\end{equation}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_unit_pulse.pdf}
	\caption{Unit-area pulse with duration $\tau$.}
	\label{fig:unit_pulse}
\end{figure}

\subsubsection{Impulse Function}

Shock loads are a common source of vibration in mechanical systems. These short-duration forces are called impulses. An impulse is a nonperiodic input that idealizes a force applied over a very short, or infinitesimal, interval. The unit impulse is denoted by $\delta(t)$ and is also called the Dirac function, Dirac delta function, Dirac impulse function, or delta function. It is obtained from the unit-area pulse $\rho(t;\tau)$ by allowing the pulse duration to approach zero while preserving its area:
\begin{equation}
	\begin{aligned}
		\delta(t) &=
		\lim_{\tau\rightarrow 0^{+}}\rho(t;\tau),
		&
		\int_{-\infty}^{\infty}\delta(t)\,\mathrm{d}t &= 1.
	\end{aligned}
	\label{eq:unit_impulse_definition}
\end{equation}

The arrow in figure \ref{fig:unit_impulse} represents concentration at $t=0$; its height is not a finite function value. The unit area carried by the impulse is the physically meaningful quantity.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_unit_impulse.pdf}
	\caption{Unit impulse concentrated at $t=0$.}
	\label{fig:unit_impulse}
\end{figure}

\subsubsection{Ramp Function}

A ramp function is zero for $t<0$ and increases linearly with time for $t\geq 0$. For example, when a hopper is filled at a constant mass-flow rate, the weight it supports increases approximately as a ramp. In a motion control system, a ramp can also represent the position command for a stage moving at constant velocity. The unit ramp has a slope of one and may be written as
\begin{equation}
	r(t)=
	\begin{cases}
		0, & t<0,\\
	...
	\end{cases}
	\qquad r(t)=t\Phi(t).
	\label{eq:unit_ramp_definition}
\end{equation}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_unit_ramp.pdf}
...
\end{figure}
	...
	\end{enumerate}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_poles_zeros_complex_plane.pdf}
				\caption{Poles and zeros of an example transfer function shown as (a) the magnitude above the complex $s$-plane and (b) the corresponding pole--zero map.}
	...
	Control systems can be designed simply by assigning specific values to the poles and zeros of the system. Physically realizable control systems must have a number of poles greater than the number of zeros. Systems that satisfy this relationship are called Proper. For a rational polynomial transfer function: The poles of a transfer function are defined as the roots of the denominator polynomial of the transfer function.  The zeros of a transfer function are simply the roots of the numerator polynomial. Visualizing of poles and zeros in the complex space can be helpful in understanding the stability of the system. Figure~\ref{fig:transfer_function_poles_and_zeros_and_3D_space} shows the complex space for an arbitrary transfer function $\sfrac{(s+2)(s-3)}{s^2+4s+20}$ where figure~\ref{fig:transfer_function_poles_and_zeros_and_3D_space}(a) shows the 3D space while figure~\ref{fig:transfer_function_poles_and_zeros_and_3D_space}(b) just reports the poles and zeros of the transfer function.  Figure~\ref{fig:transfer_function_3D_space_3_examples} reports the transfer functions for 3 additional transfer functions.  
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_transfer_function_surfaces.pdf}
				\caption{Transfer-function magnitude above the complex $s$-plane for three examples: (a) a zero at $s=1$; (b) a pole at $s=1$; and (c) a zero at $s=1$ with a pole at $s=-2$.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_massless_spring_damper.pdf}
	\caption{Massless spring--damper system with an applied force $F(t)$, showing (a) the physical configuration and (b) the free-body diagram of the rigid connector.}
\end{figure}	
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../figures/generated/ch02/ch02_first_order_free_decay.pdf}
	\caption{Normalized free decay of a first-order 1-DOF system, with the response at one time constant identified.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_first_order_stable.pdf}
	\caption{Pole location and free response of a stable first-order system.}
	\label{fig:first_order_stable}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_first_order_marginal.pdf}
	\caption{Pole location and free response of a marginal first-order system.}
	\label{fig:first_order_marginal}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_first_order_unstable.pdf}
	\caption{Pole location and free response of an unstable first-order system.}
	\label{fig:first_order_unstable}
\end{figure}
...
Consider the system:
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_forced_spring_mass.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_harmonic_motion.pdf}
...
\noindent where $x_0$ and $v_0$ are the is the displacement and velocity at $t$=0 (i.e. the initial displacement). 
		...
		\begin{review}[Euler's Formula]
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_eulers_formula.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_forced_spring_mass_damper.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_decay_and_oscillation_factors.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_damped_free_response.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_second_order_stable.pdf}
	\caption{Pole locations and decaying response of a stable second-order system.}
	\label{fig:second_order_stable}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_second_order_marginal.pdf}
	\caption{Pole locations and sustained response of a marginal second-order system.}
	\label{fig:second_order_marginal}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_second_order_unstable.pdf}
	\caption{Pole locations and growing response of an unstable second-order system.}
	\label{fig:second_order_unstable}
\end{figure}

For an unforced second-order system, the damping ratio $\zeta$ determines
whether the free response oscillates and how quickly it returns to
equilibrium. The characteristic poles are
\begin{equation}
	p_{1,2}
	=
	-\zeta\omega_\text{n}
	\pm
	\omega_\text{n}\sqrt{\zeta^2-1}.
\end{equation}
As $\zeta$ changes, these poles may be a complex-conjugate pair, a repeated
real pole, or two distinct real poles. Figure~\ref{fig:damping_responses}
compares the corresponding responses for the same normalized initial
displacement and zero initial velocity.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../figures/generated/ch02/ch02_damping_responses.pdf}
	\caption{Representative free responses of underdamped, critically damped,
	and overdamped second-order systems.}
	\label{fig:damping_responses}
\end{figure}

\noindent
\textbf{Underdamped response ($0<\zeta<1$).}
The poles are complex conjugates, so the system oscillates while its
amplitude decreases exponentially. The response may be written as
\begin{equation}
	x(t)
	=
	C e^{-\zeta\omega_\text{n}t}
	\sin\!\left(\omega_\text{d}t+\phi\right),
	\qquad
	\omega_\text{d}
	=
	\omega_\text{n}\sqrt{1-\zeta^2}.
\end{equation}

\noindent
\textbf{Critically damped response ($\zeta=1$).}
The system has a repeated pole at $p=-\omega_\text{n}$ and returns to
equilibrium without oscillating. This is the fastest nonoscillatory response:
\begin{equation}
	x(t)
	=
	\left(C_1+C_2t\right)e^{-\omega_\text{n}t}.
\end{equation}

\noindent
\textbf{Overdamped response ($\zeta>1$).}
The system has two distinct negative real poles. Its response is the sum of
...
\begin{equation}
	...
	C_1e^{p_1t}+C_2e^{p_2t},
\end{equation}
and it returns to equilibrium without oscillating, but more slowly than the
...
\begin{figure}[p]
	...
	\includegraphics[
		...
		height=210mm,
		keepaspectratio
	]{../figures/generated/ch02/ch02_second_order_pole_response_summary.pdf}
	\caption{Relationship between pole location and the free response of a
	second-order system. Pole configurations in the left half-plane are stable,
	poles on the imaginary axis are critically stable, and poles in the right
	half-plane are unstable.}
	\label{fig:second_order-pole-response-summary}
\end{figure}
\clearpage

\begin{example}[Stability of Second-Order Systems]
